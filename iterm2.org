...
- [[#implementation][Implementation]]
  ...
  - [[#iterm2-commands][iTerm2 commands]]
    ...
    - [[#set-iterm2-user-variables][Set iTerm2 user variables]]
...
*Note:* at the moment the context menu you get by right-clicking the mark at a prompt is a bit wrong because it captures the prompt as part of the command, but at the moment there is no way in Elvish to insert the marker in the correct place. The "Select command output" menu option works well.
...
The =init= function inserts the corresponding FTCS commands in the appropriate places for the prompt and the command line.

Note that this is at the moment not ideal because it captures the prompt as part of the command, but at the moment there is no way in Elvish to insert the marker in the correct place.
