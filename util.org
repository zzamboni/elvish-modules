...
We define a default version of the function which uses the external =head= command to read a line.

#+begin_src elvish
  readline~ = { put (head -n1) }
#+end_src

In recent versions of Elvish, the =read-upto= function can be used to read a line of text without invoking an external command.
